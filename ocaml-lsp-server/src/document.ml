...
let source t = Lazy.force t.msource
        ...
        let async_make_pipeline =
          Scheduler.async_exn thread (fun () ->
              let config =
                ...
                make_config uri
              in
              Mpipeline.make config (Lazy.force source))
        in
        let open Fiber.O in
...
let update_text ?version doc changes =
  ...
  in
  let msource, pipeline = make_pipeline doc.merlin tdoc in
  { doc with tdoc; pipeline; msource }
